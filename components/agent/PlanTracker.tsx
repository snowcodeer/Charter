...
export function PlanTracker({ steps }: PlanTrackerProps) {
  ...
  return (
    <>
      <aside className="rounded border border-[#4a3728] bg-[#1a1410]/70 p-3 space-y-2">
        ...
        <div className="space-y-2 max-h-[70vh] overflow-y-auto pr-1">
          {steps.map((step) => (
            <div key={step.id} className="rounded border border-[#4a3728]/80 bg-[#2a1f18]/40 p-2.5 space-y-2">
              <div className="flex items-center gap-2">
                {step.status === 'done' ? (
                  <span className="text-[#6b8f71] text-xs">✓</span>
                ) : step.status === 'active' ? (
                  <span className="w-3.5 h-3.5 rounded-full border-2 border-[#8b6f47] border-t-transparent animate-spin" />
                ) : step.status === 'error' ? (
                  <span className="text-[#9e4a3a] text-xs">!</span>
                ) : (
              ...
              {step.summary && <p className="text-[10px] text-[#6b8f71]/80">{step.summary}</p>}

              {step.screenshot && (
                <button
                  type="button"
                  onClick={() => setPreview(step)}
                  className="block w-full overflow-hidden rounded border border-[#4a3728] hover:border-[#6b5344]"
                >
      ...
      {preview?.screenshot && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1a1410]/70 p-6" onClick={() => setPreview(null)}>
          <div className="max-w-4xl max-h-[85vh] w-full rounded border border-[#6b5344] overflow-hidden bg-[#1a1410]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-3 border-t border-[#4a3728]">
              ...
              {preview.summary && <p className="text-xs text-[#6b8f71] mt-1">{preview.summary}</p>}
            </div>
          </div>
        </div>
      )}
