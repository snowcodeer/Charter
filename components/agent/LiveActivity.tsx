...
export function LiveActivity({ liveActivity, lastScan, streamingText, isLoading }: LiveActivityProps) {
    ...
    return (
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 backdrop-blur-sm p-4 animate-[slide-up_240ms_ease-out]">
        <div className="flex items-center justify-between gap-2">
          <p className="text-sm text-zinc-200 font-medium">{liveActivity.name}</p>
        ...
        {detail && <p className="mt-2 text-xs text-zinc-400 truncate">→ {detail}</p>}
  ...
  if (streamingText) {
    ...
    return (
      <div className="rounded-xl border border-zinc-800 bg-zinc-950/40 p-4">
        <p className="text-sm text-zinc-200 whitespace-pre-wrap leading-relaxed">{formatted}</p>
      </div>
    )
  }
  ...
  if (isLoading) {
    return (
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 backdrop-blur-sm p-4">
